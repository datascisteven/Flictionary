...
def view_image(img, filename = 'image'):
    ...
    ax.imshow(img.reshape(80, 80).squeeze())
    ax.axis('off')
...
def convert_to_PIL(img):
    ...
    """
    img_r = img.reshape(80,80)

    pil_img = Image.new('RGB', (80, 80), 'white')
    pixels = pil_img.load()
...
def rotate_image(src_im, angle = 45, size = (80,80)):
    """
...
#         X_train_new = np.append(X_train_new, rotated.reshape(1, 6400), axis = 0)
#         X_train_new = np.append(X_train_new, flipped.reshape(1, 6400), axis = 0)
...
#     ax.imshow(X_mean.reshape(80, 80).squeeze())
#     ax.axis('off')
...
#         image = X.iloc[indices[0][label_num]].as_matrix().reshape(80,80)  #reshape images
...
def crop_image(image):
    ...
    #resize to 80,80
    dst_im.thumbnail((80,80), Image.ANTIALIAS)
